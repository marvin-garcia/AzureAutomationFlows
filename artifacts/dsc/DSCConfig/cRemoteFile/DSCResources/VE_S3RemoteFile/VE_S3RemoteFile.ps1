...
function Set-TargetResource {
    ...
    InvokeS3ClientDownload @PSBoundParameters;
...
function InvokeS3ClientDownload {
    param (
        ...
        [Parameter(Mandatory)] [System.String] $Region,
        [Parameter(Mandatory)] [System.String] $BucketName,
    ...
    try {
        ...
        [System.UInt32] $contentLength = $awsRequestResponse.ResponseStream.Length;
        [System.Byte[]] $buffer = New-Object System.Byte[] 4096;
        [System.UInt32] $bytesRead = 0;
        [System.UInt32] $totalBytes = 0;
        do {
            $bytesRead = $awsRequestResponse.ResponseStream.Read($buffer, 0, $buffer.Length);
