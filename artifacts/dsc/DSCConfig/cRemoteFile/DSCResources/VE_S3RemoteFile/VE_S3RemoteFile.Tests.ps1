...
$dummyCredential = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList 'dummyUser', ('dummyPassword' | ConvertTo-SecureString -AsPlainText -Force);

Describe 'cS3RemoteFile\Get-TargetResource' {
...
Describe 'cS3RemoteFile\Test-TargetResource' {
    $testDriveRootPath = (Get-PSDrive -Name TestDrive).Root;
    $testFilePath = "$testDriveRootPath\README.md";
    $testFileChecksum = '401093CED51779B997248EFC3C419F15';
    $resourceParams = @{
...
Describe 'cS3RemoteFile\Set-TargetResource' {
    $testDriveRootPath = (Get-PSDrive -Name TestDrive).Root;
    $testFilePath = "$testDriveRootPath\README.md";
    $testFileChecksum = '401093CED51779B997248EFC3C419F15';
    
    Mock -CommandName 'TestAwsTools' -Verifiable -MockWith { return $true; }

    Mock -CommandName 'InvokeS3ClientDownload' -Verifiable -MockWith {
        $testFileBytes | Set-Content -Path $testFilePath -Encoding Byte -Force;
    }

    It 'Invokes InvokeS3ClientDownload and creates a checksum file.' {
        $resourceParams = @{
            DestinationPath = $testFilePath;
            Region = 'eu-central-1';
            BucketName = 'cS3RemoteFile';
